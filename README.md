...
- `recovery` now separates replayable local actions from broader recovery state with `pendingReplayCount`, `pendingImportCount`, `pendingImportHeights`, `needsRecovery`, and recent snapshot-restore metadata
- rejected peer block imports now persist a pending `block_import` recovery action instead of only leaving a transient error path
- peer-sync snapshot restore now preserves the local node's own recovery and diagnostic history, completes blocked import actions through the restored height, and records a durable `snapshot_restore` action instead of overwriting operator context with peer-local WAL state
- focused tests now cover direct import rejection, peer-sync fallback to snapshot restore, and peer-snapshot restore preserving local recovery plus diagnostics
...
- broader consensus recovery coverage beyond the current local proposal, vote, peer-import, and snapshot-recovery surfaces
...
- the round engine now supports timeout-driven proposer rotation, latest-artifact rebroadcast after link recovery, richer `roundEvidence`, per-height `roundHistory`, `blockReadiness`, import-aware `recovery`, bounded rejection diagnostics, and local consensus-action WAL replay across restart, but broader recovery tooling is still missing
...
2. Extend the new `blockReadiness`, `roundHistory`, `roundEvidence`, `recovery`, and `diagnostics` surfaces into broader recovery, peer-import diagnosis, and production incident tooling.
