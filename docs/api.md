...
- `settlementThroughput` adds the structured settlement queue-drain view, including whether monitoring is applicable, the current health-style status, active alert metadata plus `observedAt` when present, the latest commit age, backlog lag, normalized warn and fail utilization ratios, and the expected, warn, or fail thresholds derived from automatic block production
