...
- operator drills around peer identity, peer admission, delayed peer recovery, durable per-peer incident history, cross-peer incident summaries, machine-readable `/v1/metrics`, partial quorum, reproposal, per-height round history, block readiness, pending import backlog, snapshot-restore history, rejection diagnostics, and state catch-up
...
- structured logs and richer exported metrics on top of the current operator evidence, durable incident history, cross-peer summaries, and production incident tooling
