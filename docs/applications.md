...
- operator drills around peer identity, peer admission, delayed peer recovery, durable per-peer incident history, cross-peer incident summaries by state, reason, and error code, restart-safe import, snapshot, and replication-failure context in `/v1/peers`, machine-readable `/v1/metrics` including rolling throughput windows, Prometheus `/metrics` including recent TPS gauges, derived `/v1/health`, derived `/v1/alerts`, derived `/v1/slo`, recommended `/v1/alert-rules`, exported `/v1/alert-rules/prometheus`, recommended `/v1/recording-rules`, exported `/v1/recording-rules/prometheus`, recommended `/v1/dashboards`, exported `/v1/dashboards/grafana`, structured event logs, failed proposal or vote dissemination, partial quorum, reproposal, per-height round history, block readiness, durable local `block_commit` history, pending import backlog, snapshot-restore history, rejection diagnostics, state catch-up, operator throughput baselining, and queued-settlement slowdown or stall diagnosis
- product prototyping for applications that need auditable transfers plus predictable validator coordination
- architecture work for teams that want to design on top of a Rust-first WASM and confidential-compute roadmap before those phases land, including a likely path where confidential compute scales through separate worker lanes before any full consensus-sharding decision
