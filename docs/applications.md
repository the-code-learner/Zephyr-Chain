...
- `/v1/health`, `/v1/alerts`, `/v1/slo`, `/v1/alert-rules`, `/v1/alert-rules/prometheus`, `/v1/recording-rules`, `/v1/recording-rules/prometheus`, `/v1/dashboards`, `/v1/dashboards/grafana`, `/v1/metrics`, Prometheus `/metrics`, structured logs, and durable peer incident history with cross-peer state, reason, and error-code rollups give operators a concrete base for automated health checks, derived alert polling, objective tracking, rule export, dashboard bundle export, scraping, dashboard rollups, and incident response
