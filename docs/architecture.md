...
- Prometheus-style scrape export through `GET /metrics`, including per-peer retained incident counts, latest observation timestamps, committed-chain throughput gauges plus rolling `1m`, `5m`, and `15m` TPS windows, and raw settlement queue-drain lag plus threshold, utilization-ratio, and backlog-drain-estimate gauges
