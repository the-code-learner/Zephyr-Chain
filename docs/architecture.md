...
- a Go node API that validates transactions, persists chain state, produces blocks, replicates state to configured peers, exposes machine-readable readiness, derived alerts, JSON metrics, Prometheus-style scrape metrics, and can emit structured JSON incident logs
...
- derived peer-sync summaries that roll those incidents up by peer and state for operator inspection
...
- expose round-evidence state, per-height round history, block readiness, deadlines, leading tallies, quorum remaining, warning flags, local vote presence, certificate presence, local recovery state, pending import backlog, recent snapshot-restore metadata, recent rejection diagnostics, durable peer-sync history, derived peer-sync summary, machine-readable JSON metrics, Prometheus-style scrape metrics, derived readiness checks, and derived alert state through the status, consensus, block-template, `/v1/metrics`, `/metrics`, `/v1/health`, and `/v1/alerts` APIs
