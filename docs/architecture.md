...
- derived readiness through `GET /v1/health`, including settlement queue-drain checks when automatic block production is enabled, peak drain-forecast detail from recent throughput windows, and recent peer-incident horizon detail in the `peer_sync` check
- derived alerts through `GET /v1/alerts`, including settlement-throughput reduced or stalled signals when queued transactions outlive the expected block window, with detail carrying the current worst-case drain forecast, plus the aggregate peer snapshot-restore warning and repair-path-specific divergence, import-repair, and fetch-fallback snapshot warnings when retained incidents show snapshot-based repair pressure
- derived SLO summaries through `GET /v1/slo`, including the settlement-throughput objective alongside readiness and peer-continuity summaries, with settlement detail preserving the same peak drain forecast context
- recommended alert-rule bundles through `GET /v1/alert-rules` and `GET /v1/alert-rules/prometheus`, including the targeted `ZephyrPeerSnapshotRestore` peer-repair rule plus repair-path-specific snapshot-restore rules for divergence, import repair, and fetch fallback in the peer-sync group
- recommended recording-rule bundles through `GET /v1/recording-rules` and `GET /v1/recording-rules/prometheus`, including settlement-throughput state rollups, normalized settlement queue-drain utilization, projected queue-drain pressure, max projected queue-drain pressure, queue-drain estimate, and max queue-drain estimate rollups, the per-peer incident-pressure rollups `zephyr:peer_sync:incident_pressure_by_peer` and `zephyr:peer_sync:incident_pressure_by_horizon`, the per-peer snapshot-repair rollups `zephyr:peer_sync:snapshot_restore_pressure_by_peer` and `zephyr:peer_sync:snapshot_restore_age_by_peer`, the peer snapshot-restore pressure rollup `zephyr:peer_sync:snapshot_restore_pressure`, the repair-path rollup `zephyr:peer_sync:snapshot_restore_pressure_by_reason`, canonical recent-TPS rollups for the overview dashboard, and runtime-aware disabled reasons when a producing or synced role is not active, with related-alert metadata now preserving both the compatibility aggregate `peer_snapshot_restored` code and the split `peer_snapshot_restore_*` codes for snapshot-repair rollups
- recommended dashboard bundles through `GET /v1/dashboards` and `GET /v1/dashboards/grafana`, including settlement-throughput state, raw settlement queue-drain lag, normalized queue-drain utilization, recording-rule-backed estimated queue-drain pressure, a recording-rule-backed worst-case projected-pressure stat, recording-rule-backed estimated queue-drain time, a recording-rule-backed worst-case estimated queue-drain time stat, and recent transaction throughput in the overview bundle plus peer incident-pressure drill-down by peer, peer incident pressure horizons, peer snapshot-restore pressure by peer, peer snapshot-restore heights, peer snapshot-restore age, peer snapshot-restore pressure, and peer snapshot-restore reasons in the peer-sync bundle, with JSON metadata preserved for runtime-disabled panels, enabled-only Grafana export, and related-alert metadata that keeps the aggregate plus split snapshot-repair codes aligned on the peer-sync panels
- runtime status through `GET /v1/status`
- machine-readable observability through `GET /v1/metrics`, including structured settlement alert metadata, normalized queue-drain utilization ratios, recent backlog-drain estimates, per-estimate warn utilization ratios, an explicit peak drain-estimate summary, and fixed recent peer-incident horizons for `5m`, `15m`, `1h`, `6h`, and `24h`
