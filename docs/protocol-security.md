...
The state root is a deterministic `zephyr/state/v1` commitment over consensus-critical committed account state and the active validator snapshot. Volatile node-local telemetry, pending diagnostics, and local timing metadata are excluded.
...
After a quorum-approved restore, the node restores only verified committed state. Remote mempool contents, pending round state, pending proposals/votes, and remote diagnostics are not blindly adopted; local volatile state is restarted from a clean recovery boundary.
