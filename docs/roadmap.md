...
- broader recovery, dashboard-friendly export adapters, recording rules, and transport-facing incident evidence beyond the current local round history, block readiness, warnings, durable peer-sync history, derived peer-sync summary, JSON metrics, Prometheus `GET /metrics`, `GET /v1/health`, `GET /v1/alerts`, `GET /v1/slo`, `GET /v1/alert-rules`, `GET /v1/alert-rules/prometheus`, structured event logs, import backlog, snapshot-restore history, and rejection history with only bounded retention so far
...
5. Tune the new alert-rule bundles, add dashboard or recording-rule exports on top of them, and extend the current structured logs, JSON metrics, Prometheus `GET /metrics`, `GET /v1/alerts`, `GET /v1/slo`, and readiness surfaces into wider aggregation beyond the current bounded peer-history window.
...
5. Extend the current JSON metrics surface, Prometheus `GET /metrics`, `GET /v1/health`, `GET /v1/alerts`, `GET /v1/slo`, `GET /v1/alert-rules`, `GET /v1/alert-rules/prometheus`, and structured event logs into dashboard bundles, broader multi-peer incident aggregation, recording rules, and export adapters for validator, sync, admission, transport, automation, and repair operations.
