...
1. Extend the new `blockReadiness`, `roundHistory`, `roundEvidence`, `recovery`, `diagnostics`, `peerSyncHistory`, `peerSyncSummary`, per-peer `recentIncidents`, and `GET /v1/health` checks into deeper peer-import, divergence, longer-horizon multi-peer diagnosis, and production incident flows.
...
- metrics, readiness probes, tracing, dashboards, and incident-friendly logs
