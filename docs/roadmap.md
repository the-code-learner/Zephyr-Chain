...
- recommended recording-rule bundle exports through JSON `GET /v1/recording-rules` and Prometheus-oriented `GET /v1/recording-rules/prometheus`, now including settlement-throughput state rollups plus normalized queue-drain utilization rollups for queue-drain dashboards, a per-peer incident-pressure rollup for peer-sync dashboards, canonical `1m`, `5m`, and `15m` TPS rollups for the overview dashboard, and runtime-aware disabled reasons when settlement monitoring or peer sync is not applicable
