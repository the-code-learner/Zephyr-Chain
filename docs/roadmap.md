...
- broader recovery, alerting-friendly metrics export, and transport-facing incident evidence beyond the current local round history, block readiness, warnings, durable peer-sync history, derived peer-sync summary, JSON metrics, structured event logs, import backlog, snapshot-restore history, and rejection history with only bounded retention so far
...
5. Extend the current JSON metrics surface and structured event logs into richer exported metrics, health surfaces, and broader multi-peer incident aggregation for validator, sync, admission, transport, automation, and repair operations.
