...
1. Extend the new `blockReadiness`, `roundHistory`, `roundEvidence`, `recovery`, `diagnostics`, `peerSyncHistory`, `peerSyncSummary`, per-peer `recentIncidents`, `GET /v1/health`, `GET /v1/alerts`, `GET /v1/slo`, `GET /v1/alert-rules`, `GET /v1/alert-rules/prometheus`, `GET /v1/recording-rules`, `GET /v1/recording-rules/prometheus`, `GET /v1/dashboards`, and `GET /v1/dashboards/grafana` into deeper peer-import, divergence, longer-horizon multi-peer diagnosis, and production incident flows.
   ...
   1. extend the same operator surfaces with horizon-aware peer-summary views so operators can compare current retained pressure against longer windows instead of only the bounded retained window;
   2. add deeper peer-level repair drill-down panels that correlate `recentIncidents`, latest repair metadata, and pressure rollups without rewriting PromQL downstream;
   3. decide whether the compact `/v1/alerts` surface should stay aggregated on `peer_snapshot_restored` or grow repair-path-specific alert codes once longer-horizon evidence exists.
...
5. Extend the current JSON metrics surface, Prometheus `GET /metrics`, `GET /v1/health`, `GET /v1/alerts`, `GET /v1/slo`, `GET /v1/alert-rules`, `GET /v1/alert-rules/prometheus`, `GET /v1/recording-rules`, `GET /v1/recording-rules/prometheus`, `GET /v1/dashboards`, `GET /v1/dashboards/grafana`, and structured event logs into broader dashboard packages, multi-peer incident aggregation, and export adapters for validator, sync, admission, transport, automation, and repair operations.
   Progress now: repair-path-specific snapshot-restore rollups, panels, and recommended alert rules already live under the existing peer-sync bundle, so the next slice can stay focused on richer history and drill-down instead of creating a new dashboard family.
   Immediate observability slice after this step:
   1. surface longer-horizon peer pressure summaries that keep JSON metadata and cleanly degrade in Grafana export;
   2. enrich the peer-sync bundle with deeper peer-level repair drill-down that ties retained pressure to latest snapshot metadata without custom downstream joins;
