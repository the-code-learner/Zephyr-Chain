...
- those same responses now expose `peerSyncSummary`, which rolls those incidents up by peer, state, reason, and error code plus fixed recent `5m`, `15m`, and `1h` horizons so operators can see the dominant network problem quickly
