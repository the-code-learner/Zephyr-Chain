...
- `/v1/recording-rules` returns readiness, consensus, throughput, peer-sync, and operator-summary recording-rule groups with stable `record` names, expressions, source metrics, and disabled reasons when a rule is not applicable to the current node configuration; the throughput group includes `zephyr:settlement_throughput:at_risk`, `zephyr:settlement_throughput:breached`, `zephyr:settlement_queue_drain:warn_utilization`, and `zephyr:settlement_queue_drain:fail_utilization`, the peer-sync group includes the per-peer incident-pressure rollup `zephyr:peer_sync:incident_pressure_by_peer`, and the operator-summary group includes `zephyr:chain:transactions_per_second_1m`, `zephyr:chain:transactions_per_second_5m`, and `zephyr:chain:transactions_per_second_15m`
