	...
	groups := []AlertRuleGroup{
		...
		},
		{
			...
			Rules: []AlertRule{
				...
				disableAlertRule(
					newAlertRule(
						"ZephyrPeerSnapshotRestoreDivergence",
						alertSeverityWarning,
						"peer_sync",
						"Peer divergence is triggering snapshot restore repair",
						"Retained peer incidents show snapshot restore is being used because one or more peers diverged from local chain state. Inspect /v1/alerts, /v1/status peerSyncSummary, /v1/metrics, and /v1/peers to confirm whether peers disagree on committed chain state before treating repeated repair as normal background sync.",
						"zephyr_alert_active{code=\"peer_snapshot_restore_divergence\",severity=\"warning\",component=\"peer_sync\"} == 1",
						"5m",
						[]string{"zephyr_alert_active", "zephyr_peer_sync_reason_occurrence_count"},
						[]string{"peer_snapshot_restore_divergence"},
						[]string{"peer_sync_continuity"},
					),
					peerSyncDisabledReason,
				),
				disableAlertRule(
					newAlertRule(
						"ZephyrPeerSnapshotRestoreFetchFallback",
						alertSeverityWarning,
						"peer_sync",
						"Peer block fetch fallback is triggering snapshot restore",
						"Retained peer incidents show snapshot restore is being used because block fetch by height fell back to snapshot repair. Inspect /v1/alerts, /v1/status peerSyncSummary, /v1/metrics, and /v1/peers to separate transient fetch churn from deeper sync faults.",
						"zephyr_alert_active{code=\"peer_snapshot_restore_fetch_fallback\",severity=\"warning\",component=\"peer_sync\"} == 1",
						"5m",
						[]string{"zephyr_alert_active", "zephyr_peer_sync_reason_occurrence_count"},
						[]string{"peer_snapshot_restore_fetch_fallback"},
						[]string{"peer_sync_continuity"},
					),
					peerSyncDisabledReason,
				),
				disableAlertRule(
					newAlertRule(
						"ZephyrPeerSnapshotRestoreImportRepair",
						alertSeverityWarning,
						"peer_sync",
						"Peer import repair is triggering snapshot restore",
						"Retained peer incidents show snapshot restore is being used after block import rejection on one or more peers. Inspect /v1/alerts, /v1/status peerSyncSummary, /v1/metrics, and /v1/peers to separate certificate or template mismatches from transport delay.",
						"zephyr_alert_active{code=\"peer_snapshot_restore_import_repair\",severity=\"warning\",component=\"peer_sync\"} == 1",
						"5m",
						[]string{"zephyr_alert_active", "zephyr_peer_sync_reason_occurrence_count"},
						[]string{"peer_snapshot_restore_import_repair"},
						[]string{"peer_sync_continuity"},
					),
					peerSyncDisabledReason,
				),
