	...
	return []Dashboard{
		{
			...
			Panels: []DashboardPanel{
				...
				disableDashboardPanel(newDashboardPanel(
					...
					"Uses the structured settlement drain estimate gauge so operators can compare how long the current mempool would take to clear at the recent 1m, 5m, and 15m committed throughput baselines; windows with no recent throughput baseline will show no data.",
					"s",
					[]DashboardQuery{
						{Ref: "A", Expression: "zephyr_settlement_estimated_queue_drain_seconds{window=\"1m\"}", Legend: "estimate_1m"},
						{Ref: "B", Expression: "zephyr_settlement_estimated_queue_drain_seconds{window=\"5m\"}", Legend: "estimate_5m"},
						{Ref: "C", Expression: "zephyr_settlement_estimated_queue_drain_seconds{window=\"15m\"}", Legend: "estimate_15m"},
					},
					[]string{"zephyr_settlement_estimated_queue_drain_seconds"},
					[]string{"/metrics", "/v1/metrics"},
					nil,
					[]string{settlementThroughputAlertReduced, settlementThroughputAlertStalled},
					[]string{"settlement_throughput"},
				), throughputDisabledReason),
