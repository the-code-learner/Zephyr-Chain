	...
	return []Dashboard{
		{
			...
			Panels: []DashboardPanel{
				...
				disableDashboardPanel(newDashboardPanel(
					...
					"timeseries",
					"Current queue-drain lag plotted against the warn and fail thresholds.",
					"Shows the raw backlog lag in seconds beside the warn and fail thresholds derived from the configured automatic block cadence so operators can see pressure building before the settlement-throughput state flips.",
					"s",
					[]DashboardQuery{
						{Ref: "A", Expression: "zephyr_settlement_queue_drain_lag_seconds", Legend: "lag_seconds"},
						{Ref: "B", Expression: "zephyr_settlement_queue_drain_threshold_seconds{threshold=\"warn\"}", Legend: "warn_threshold"},
						{Ref: "C", Expression: "zephyr_settlement_queue_drain_threshold_seconds{threshold=\"fail\"}", Legend: "fail_threshold"},
					},
					[]string{"zephyr_settlement_queue_drain_lag_seconds", "zephyr_settlement_queue_drain_threshold_seconds"},
					[]string{"/metrics", "/v1/metrics", "/v1/health"},
					nil,
					[]string{settlementThroughputAlertReduced, settlementThroughputAlertStalled},
					[]string{"settlement_throughput"},
				), throughputDisabledReason),
				disableDashboardPanel(newDashboardPanel(
					...
					"bargauge",
					"Normalized queue-drain lag against the warn and fail thresholds.",
					"Uses canonical queue-drain utilization recording rules so operators can compare how close the current lag is to the warn and fail thresholds without re-encoding block-interval math in the dashboard layer.",
					"percentunit",
					[]DashboardQuery{
						{Ref: "A", Expression: "zephyr:settlement_queue_drain:warn_utilization", Legend: "warn_ratio"},
						{Ref: "B", Expression: "zephyr:settlement_queue_drain:fail_utilization", Legend: "fail_ratio"},
					},
				...
				disableDashboardPanel(newDashboardPanel(
					"settlement_queue_drain_estimate",
					"Estimated queue-drain time",
					"bargauge",
					"Estimated seconds to clear the current queued backlog at recent throughput.",
					"Uses canonical settlement drain-estimate recording rules so operators can compare how long the current mempool would take to clear at the recent 1m, 5m, and 15m committed throughput baselines without pinning dashboards directly to the raw estimate gauge.",
					"s",
					[]DashboardQuery{
						{Ref: "A", Expression: "zephyr:settlement_queue_drain:estimate_seconds_1m", Legend: "estimate_1m"},
						{Ref: "B", Expression: "zephyr:settlement_queue_drain:estimate_seconds_5m", Legend: "estimate_5m"},
						{Ref: "C", Expression: "zephyr:settlement_queue_drain:estimate_seconds_15m", Legend: "estimate_15m"},
					},
					[]string{"zephyr:settlement_queue_drain:estimate_seconds_1m", "zephyr:settlement_queue_drain:estimate_seconds_5m", "zephyr:settlement_queue_drain:estimate_seconds_15m"},
					[]string{"/metrics", "/v1/metrics", "/v1/recording-rules"},
					[]string{"zephyr:settlement_queue_drain:estimate_seconds_1m", "zephyr:settlement_queue_drain:estimate_seconds_5m", "zephyr:settlement_queue_drain:estimate_seconds_15m"},
					[]string{settlementThroughputAlertReduced, settlementThroughputAlertStalled},
					[]string{"settlement_throughput"},
				...
				disableDashboardPanel(newDashboardPanel(
					"settlement_queue_drain_estimate_peak_time",
					"Worst-case estimated queue-drain time",
					"stat",
					"Highest projected backlog-clear time across recent throughput windows.",
					"Uses the canonical max drain-time recording rule so operators can see at a glance the slowest recent settlement projection without scanning all three throughput windows.",
					"s",
					[]DashboardQuery{{Ref: "A", Expression: "zephyr:settlement_queue_drain:estimate_seconds_max", Legend: "max_estimate_seconds"}},
					[]string{"zephyr_settlement_estimated_queue_drain_seconds"},
					[]string{"/metrics", "/v1/metrics", "/v1/recording-rules"},
					[]string{"zephyr:settlement_queue_drain:estimate_seconds_max"},
					[]string{settlementThroughputAlertReduced, settlementThroughputAlertStalled},
					[]string{"settlement_throughput"},
				), throughputDisabledReason),
				disableDashboardPanel(newDashboardPanel(
					"settlement_queue_drain_estimate_pressure",
					"Estimated queue-drain pressure",
					"bargauge",
					"Estimated backlog-clear time normalized against the warn threshold across throughput windows.",
					"Uses canonical estimate-utilization recording rules so operators can compare whether the current backlog would clear within the healthy queue-drain envelope at the recent 1m, 5m, and 15m committed throughput baselines.",
					"percentunit",
					[]DashboardQuery{
						{Ref: "A", Expression: "zephyr:settlement_queue_drain:estimate_warn_utilization_1m", Legend: "warn_ratio_1m"},
						{Ref: "B", Expression: "zephyr:settlement_queue_drain:estimate_warn_utilization_5m", Legend: "warn_ratio_5m"},
						{Ref: "C", Expression: "zephyr:settlement_queue_drain:estimate_warn_utilization_15m", Legend: "warn_ratio_15m"},
					},
					[]string{"zephyr_settlement_estimated_queue_drain_warn_utilization_ratio"},
					[]string{"/metrics", "/v1/metrics", "/v1/recording-rules"},
					[]string{"zephyr:settlement_queue_drain:estimate_warn_utilization_1m", "zephyr:settlement_queue_drain:estimate_warn_utilization_5m", "zephyr:settlement_queue_drain:estimate_warn_utilization_15m"},
					[]string{settlementThroughputAlertReduced},
					[]string{"settlement_throughput"},
				), throughputDisabledReason),
				disableDashboardPanel(newDashboardPanel(
					"settlement_queue_drain_estimate_peak_pressure",
					"Worst-case estimated queue-drain pressure",
					"stat",
					"Highest projected queue-drain pressure across recent throughput windows.",
					"Uses the canonical max projected-pressure recording rule so operators can see at a glance whether any recent throughput baseline suggests the backlog would clear outside the warn envelope.",
					"percentunit",
					[]DashboardQuery{{Ref: "A", Expression: "zephyr:settlement_queue_drain:estimate_warn_utilization_max", Legend: "max_warn_ratio"}},
					[]string{"zephyr_settlement_estimated_queue_drain_warn_utilization_ratio"},
					[]string{"/metrics", "/v1/metrics", "/v1/recording-rules"},
					[]string{"zephyr:settlement_queue_drain:estimate_warn_utilization_max"},
					[]string{settlementThroughputAlertReduced},
					[]string{"settlement_throughput"},
				), throughputDisabledReason),
				newDashboardPanel(
		...
		disableDashboard(Dashboard{
			...
			Panels: []DashboardPanel{
				...
				newDashboardPanel(
					"peer_snapshot_restore_by_peer",
					"Peer snapshot restore pressure by peer",
					"bargauge",
					"Retained snapshot-restore incident pressure grouped by peer.",
					"Highlights which peers are driving snapshot-based repair pressure while preserving the latest retained repair reason on the series label so operators can jump from the bundle to `/v1/peers` recentIncidents and latest repair metadata without custom PromQL.",
					"none",
					[]DashboardQuery{
						{Ref: "A", Expression: "zephyr:peer_sync:snapshot_restore_pressure_by_peer", Legend: "{{peer_url}} {{latest_reason}}"},
					},
				...
				newDashboardPanel(
					"peer_snapshot_restore_heights",
					"Peer snapshot restore heights",
					"bargauge",
					"Latest snapshot-restore height grouped by peer.",
					"Pairs retained peer repair pressure with the last restored height and repair reason, while `/v1/peers` remains the source for the corresponding block hash and recent incident timeline.",
					"none",
					[]DashboardQuery{
						{Ref: "A", Expression: "zephyr_peer_snapshot_restore_last_height", Legend: "{{peer_url}} {{reason}}"},
					},
				...
				newDashboardPanel(
					...
					"stat",
					"Retained snapshot-restore incident pressure across peers.",
					"Highlights when peer repair is relying on snapshot restore, which is especially useful for spotting divergence repair and repeated import fallback before it becomes normal background behavior.",
					"none",
					[]DashboardQuery{{Ref: "A", Expression: "zephyr:peer_sync:snapshot_restore_pressure", Legend: "snapshot_restore"}},
