		...
		disableDashboard(Dashboard{
			...
			Panels: []DashboardPanel{
				...
				newDashboardPanel(
					...
					"none",
					[]DashboardQuery{{Ref: "A", Expression: "zephyr_peer_sync_peer_occurrence_count", Legend: "{{peer_url}} {{latest_state}}"}},
					[]string{"zephyr_peer_sync_peer_occurrence_count", "zephyr_peer_sync_peer_latest_observed_at_seconds"},
					[]string{"/metrics", "/v1/metrics", "/v1/status"},
					nil,
					[]string{"peer_import_blocked", "peer_admission_blocked", "peer_replication_blocked"},
					[]string{"peer_sync_continuity"},
				),
