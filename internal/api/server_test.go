...
func TestHandleBroadcastTransactionRejectsInvalidTransportIdentity(t *testing.T) {
	...
	server.Handler().ServeHTTP(recorder, request)
	if recorder.Code != http.StatusBadRequest {
		t.Fatalf("expected malformed proof status 400, got %d", recorder.Code)
	}
}
...
func TestHandleBroadcastTransactionRejectsUnboundPeerValidator(t *testing.T) {
	...
	server.Handler().ServeHTTP(recorder, request)
	if recorder.Code != http.StatusForbidden {
		t.Fatalf("expected unbound validator status 403, got %d", recorder.Code)
	}
}
...
func TestHandleConsensusProposalAndVotesExposeArtifacts(t *testing.T) {
	...
	proposal := signedConsensusProposalForServer(t, server, proposer, 1, 0, "", time.Date(2026, time.March, 23, 13, 0, 0, 0, time.UTC), []tx.Envelope{signedEnvelope(t, 5, 1, "block-1-tx")})
...
func TestHandleConsensusExposesRoundEvidence(t *testing.T) {
	...
	proposal := signedConsensusProposalForServer(t, server, proposer, 1, 0, "", time.Date(2026, time.March, 24, 14, 1, 0, 0, time.UTC), []tx.Envelope{signedEnvelope(t, 5, 1, "round-evidence")})
...
func TestHandleConsensusExposesReproposalAndTimeoutWarnings(t *testing.T) {
	...
	proposal := signedConsensusProposalForServer(t, server, proposer, 1, 0, "", startedAt.Add(5*time.Second), []tx.Envelope{signedEnvelope(t, 5, 1, "reproposal-warning")})
	if err := server.ledger.RecordProposal(proposal); err != nil {
		t.Fatalf("record proposal: %v", err)
	}
...
func TestHandleConsensusExposesRoundHistoryAcrossRounds(t *testing.T) {
	...
	proposalRound0 := signedConsensusProposalForServer(t, server, proposer, 1, 0, "", roundZeroStarted.Add(5*time.Second), []tx.Envelope{signedEnvelope(t, 5, 1, "history-round-0")})
	...
	proposalRound1 := signedConsensusProposalForServer(t, server, nextProposer, 1, 1, "", roundOneStarted.Add(5*time.Second), []tx.Envelope{signedEnvelope(t, 6, 1, "history-round-1")})
...
func TestHandleStatusExposesConsensusRecovery(t *testing.T) {
	...
	proposal := signedConsensusProposalForServer(t, server, validator, 1, 0, "", time.Date(2026, time.March, 24, 14, 30, 0, 0, time.UTC), []tx.Envelope{signedEnvelope(t, 5, 1, "recovery-view")})
...
func TestHandleBlockTemplateExposesBlockReadinessAcrossCertificateLifecycle(t *testing.T) {
	...
	proposal := signedConsensusProposalForServer(t, server, proposer, templateResponse.Block.Height, 0, templateResponse.Block.PreviousHash, templateResponse.Block.ProducedAt, templateResponse.Block.Transactions)
	proposalBody, err := json.Marshal(proposal)
	if err != nil {
		t.Fatalf("marshal proposal: %v", err)
	}
...
func TestHandleConsensusGatedProduceBlockUsesProposalBodyWithoutMempool(t *testing.T) {
	...
	proposal := signedConsensusProposalForServer(t, server, proposer, 1, 0, "", producedAt, []tx.Envelope{envelope})
	proposalBody, err := json.Marshal(proposal)
	if err != nil {
		t.Fatalf("marshal proposal: %v", err)
	}
...
func TestPeerReplicationPropagatesConsensusProposalAndVotes(t *testing.T) {
	...
	for _, target := range []*Server{mainServer, peerServer} {
		if _, err := target.ledger.Credit(proposalTx.From, 10); err != nil {
			t.Fatalf("credit proposal sender: %v", err)
		}
	}
...
func TestPeerReplicationImportsCertifiedBlockWhenConsensusRequired(t *testing.T) {
	...
	proposal := signedConsensusProposal(t, proposer, templateResponse.Block.Height, 0, templateResponse.Block.PreviousHash, templateResponse.Block.ProducedAt, templateResponse.Block.Transactions, templateResponse.Block.StateRoot)
	proposalBody, err := json.Marshal(proposal)
	if err != nil {
		t.Fatalf("marshal proposal: %v", err)
	}
...
func TestPeerSyncHistoryPersistsAcrossServerRestart(t *testing.T) {
	...
	if err := server.ledger.RecordPeerSyncIncident(ledger.PeerSyncIncident{
		...
		ErrorCode: "snapshot_quorum_required", ErrorMessage: ledger.ErrSnapshotQuorumRequired.Error(), FirstObservedAt: now, LastObservedAt: now,
	}); err != nil {
...
func TestHandleRecordingRulesExposeRecommendedBundles(t *testing.T) {
	...
	if rule, ok := recordingRuleByRecord(response.Groups, "zephyr:settlement_queue_drain:estimate_warn_utilization_max"); !ok || rule.Enabled || !strings.Contains(rule.DisabledReason, "disabled") {
		t.Fatalf("expected disabled settlement queue-drain max estimate warn-utilization recording rule, got %+v", response.Groups)
	}
	if rule, ok := recordingRuleByRecord(response.Groups, "zephyr:settlement_queue_drain:estimate_seconds_max"); !ok || rule.Enabled || !strings.Contains(rule.DisabledReason, "disabled") {
		t.Fatalf("expected disabled settlement queue-drain max estimate recording rule, got %+v", response.Groups)
	}
...
func TestHandlePrometheusRecordingRulesExportsEnabledRulesOnly(t *testing.T) {
	...
	if !strings.Contains(body, "      - record: zephyr:peer_sync:snapshot_restore_pressure_by_peer\n") {
		t.Fatalf("expected peer snapshot-restore pressure by peer recording rule in prometheus recording rules, got:\n%s", body)
	}
	...
	if !strings.Contains(body, "  - name: zephyr.throughput\n") {
		t.Fatalf("expected throughput recording rule group in prometheus recording rules, got:\n%s", body)
	}
	if !strings.Contains(body, "      - record: zephyr:settlement_throughput:breached\n") {
		t.Fatalf("expected settlement throughput breached recording rule in prometheus recording rules, got:\n%s", body)
	}
	if !strings.Contains(body, "        expr: 'zephyr_slo_objective_status{objective=\"settlement_throughput\",status=\"breached\"}'\n") {
		t.Fatalf("expected settlement throughput breached expression in prometheus recording rules, got:\n%s", body)
	}
	if !strings.Contains(body, "      - record: zephyr:settlement_queue_drain:warn_utilization\n") {
		t.Fatalf("expected settlement queue-drain warn utilization recording rule in prometheus recording rules, got:\n%s", body)
	}
	if !strings.Contains(body, "        expr: 'zephyr_settlement_queue_drain_lag_seconds / clamp_min(zephyr_settlement_queue_drain_threshold_seconds{threshold=\"warn\"}, 1)'\n") {
		t.Fatalf("expected settlement queue-drain warn utilization expression in prometheus recording rules, got:\n%s", body)
	}
	if !strings.Contains(body, "      - record: zephyr:settlement_queue_drain:estimate_seconds_5m\n") {
		t.Fatalf("expected settlement queue-drain 5m estimate recording rule in prometheus recording rules, got:\n%s", body)
	}
...
func TestHandleDashboardsExposeRecommendedBundles(t *testing.T) {
	...
	} else if panel, ok := dashboardPanelByID(dashboard.Panels, "settlement_queue_drain_estimate_peak_time"); !ok || panel.Enabled || !strings.Contains(panel.DisabledReason, "disabled") || len(panel.RecordingRules) != 1 || panel.RecordingRules[0] != "zephyr:settlement_queue_drain:estimate_seconds_max" {
		t.Fatalf("expected settlement_queue_drain_estimate_peak_time panel to be disabled with max estimate metadata intact, got %+v", dashboard.Panels)
	...
	} else if panel, ok := dashboardPanelByID(dashboard.Panels, "settlement_queue_drain_estimate_peak_pressure"); !ok || panel.Enabled || !strings.Contains(panel.DisabledReason, "disabled") || len(panel.RecordingRules) != 1 || panel.RecordingRules[0] != "zephyr:settlement_queue_drain:estimate_warn_utilization_max" {
		t.Fatalf("expected settlement_queue_drain_estimate_peak_pressure panel to be disabled with max estimate utilization metadata intact, got %+v", dashboard.Panels)
	}
	...
	} else if panel, ok := dashboardPanelByID(dashboard.Panels, "peer_snapshot_restore_by_peer"); !ok || panel.Enabled || !strings.Contains(panel.DisabledReason, "disabled") || len(panel.RecordingRules) != 1 || panel.RecordingRules[0] != "zephyr:peer_sync:snapshot_restore_pressure_by_peer" {
...
func TestHandleGrafanaDashboardsExportsEnabledDashboardsOnly(t *testing.T) {
	...
	if dashboard, ok := grafanaDashboardByName(response.Dashboards, "zephyr.peer_sync"); !ok {
		...
		panel, ok = grafanaPanelByTitle(dashboard.Dashboard.Panels, "Peer snapshot restore pressure by peer")
		if !ok || panel.Type != "bargauge" {
			t.Fatalf("expected peer snapshot restore pressure by peer bargauge panel, got %+v", dashboard.Dashboard.Panels)
		}
		if _, ok := grafanaTargetByExpression(panel.Targets, "zephyr:peer_sync:snapshot_restore_pressure_by_peer"); !ok {
			t.Fatalf("expected peer snapshot restore pressure by peer query in grafana panel, got %+v", panel.Targets)
		}
		panel, ok = grafanaPanelByTitle(dashboard.Dashboard.Panels, "Peer snapshot restore heights")
		if !ok || panel.Type != "bargauge" {
			t.Fatalf("expected peer snapshot restore heights bargauge panel, got %+v", dashboard.Dashboard.Panels)
		}
	...
	if dashboard, ok := grafanaDashboardByName(response.Dashboards, "zephyr.overview"); !ok {
		...
		panel, ok = grafanaPanelByTitle(dashboard.Dashboard.Panels, "Settlement throughput state")
		if !ok || panel.Type != "bargauge" {
			t.Fatalf("expected settlement throughput bargauge panel, got %+v", dashboard.Dashboard.Panels)
		}
		...
		panel, ok = grafanaPanelByTitle(dashboard.Dashboard.Panels, "Settlement queue-drain utilization")
		if !ok || panel.Type != "bargauge" {
			t.Fatalf("expected settlement queue-drain utilization bargauge panel, got %+v", dashboard.Dashboard.Panels)
		}
		...
		panel, ok = grafanaPanelByTitle(dashboard.Dashboard.Panels, "Worst-case estimated queue-drain time")
		if !ok || panel.Type != "stat" {
			t.Fatalf("expected worst-case estimated queue-drain time stat panel, got %+v", dashboard.Dashboard.Panels)
		}
		if _, ok := grafanaTargetByExpression(panel.Targets, "zephyr:settlement_queue_drain:estimate_seconds_max"); !ok {
			t.Fatalf("expected worst-case estimated queue-drain time query in grafana panel, got %+v", panel.Targets)
		}
		...
		panel, ok = grafanaPanelByTitle(dashboard.Dashboard.Panels, "Worst-case estimated queue-drain pressure")
		if !ok || panel.Type != "stat" {
			t.Fatalf("expected worst-case estimated queue-drain pressure stat panel, got %+v", dashboard.Dashboard.Panels)
		}
		if _, ok := grafanaTargetByExpression(panel.Targets, "zephyr:settlement_queue_drain:estimate_warn_utilization_max"); !ok {
			t.Fatalf("expected worst-case estimated queue-drain pressure query in grafana panel, got %+v", panel.Targets)
		}
	}
}
func TestHandleGrafanaDashboardsOmitDisabledSettlementAndPeerSyncPanels(t *testing.T) {
	...
	if dashboard, ok := grafanaDashboardByName(response.Dashboards, "zephyr.overview"); !ok {
		...
		if _, ok := grafanaPanelByTitle(dashboard.Dashboard.Panels, "Worst-case estimated queue-drain time"); ok {
			t.Fatalf("expected disabled worst-case estimated queue-drain time panel to be omitted, got %+v", dashboard.Dashboard.Panels)
		}
		...
		if _, ok := grafanaPanelByTitle(dashboard.Dashboard.Panels, "Worst-case estimated queue-drain pressure"); ok {
			t.Fatalf("expected disabled worst-case estimated queue-drain pressure panel to be omitted, got %+v", dashboard.Dashboard.Panels)
		}
	}
}
...
func TestHandleImportBlockRejectsAndExposesPendingImportRecovery(t *testing.T) {
	...
	proposal := signedConsensusProposalForServer(t, producer, proposer, template.Height, 0, template.PreviousHash, template.ProducedAt, template.Transactions)
	...
	request := signedPeerRequestWithSigner(t, proposer, http.MethodPost, "/v1/internal/blocks", body)
	recorder := httptest.NewRecorder()
...
func TestPeerSyncConsensusImportFailureRestoresSnapshotAndRecordsRecoveryHistory(t *testing.T) {
	...
	if _, err := producer.ledger.ProduceBlock(10); err != nil {
		t.Fatal(err)
	}
...
func TestInternalBlockEndpointRejectsMissingRequestProof(t *testing.T) {
	...
	server.Handler().ServeHTTP(recorder, request)
	if recorder.Code != http.StatusForbidden {
		t.Fatalf("expected internal endpoint to require peer proof, got %d", recorder.Code)
	}
}
...
func signedConsensusProposalForServer(t *testing.T, server *Server, signer consensusSigner, height uint64, round uint64, previousHash string, producedAt time.Time, transactions []tx.Envelope, stateRoots ...string) consensus.Proposal {
	...
	for _, envelope := range transactions {
		...
		if next < required[envelope.From] {
			t.Fatal("test fixture balance overflow")
		}
	...
	for address, amount := range required {
		...
		if view.Balance < amount {
			if _, err := server.ledger.Credit(address, amount-view.Balance); err != nil {
...
func signedPeerRequestWithSigner(t *testing.T, signer consensusSigner, method string, path string, body []byte) *http.Request {
	...
	if err != nil {
		t.Fatalf("build peer request proof: %v", err)
	}
